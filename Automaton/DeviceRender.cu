...
DeviceRender::DeviceRender(Game&& game, bool silent)
    : IRender()
...
DeviceRender::DeviceRender(const Game& game, bool silent)
    : IRender()
    , _dev_game(utils::copy_allocate_managed(game))
    , _dev_frame(utils::allocate<Color>(game.len(), false))
    , _host_frame(utils::allocate<Color>(game.len(), true))
...
__global__ void __evaluate__(const Game* game, Color* frame) {
    assert(game != nullptr);
